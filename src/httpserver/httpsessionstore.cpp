...
HttpSession HttpSessionStore::getSession(HttpRequest& request, HttpResponse& response, bool allowCreate)
{
    ...
    if (!sessionId.isEmpty())
    {
        ...
        if (!session.isNull())
        {
            ...
            QByteArray cookieComment=settings->value("cookieComment", "Identifies the user").toByteArray();
            QByteArray cookieDomain=settings->value("cookieDomain", "tsunamiWeb").toByteArray();
